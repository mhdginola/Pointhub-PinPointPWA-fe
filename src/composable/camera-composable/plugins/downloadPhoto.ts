...
function _dataURItoBlob(dataURI: any) {
...
function _padWithZeroNumber(number: any, width: any) {
  number = number + ''
  return number.length >= width ? number : new Array(width - number.length + 1).join('0') + number
}

function _getFileExtention(blobType: any) {
...
function _getFileName(prefixName: any, imageNumber: any, blobType: any) {
...
function _downloadImageFileFomBlob(blob: any, prefixName: any, imageNumber: any) {
...
export function downloadPhoto(dataUri: any, prefixName = 'photo', imageNumber = 0) {
