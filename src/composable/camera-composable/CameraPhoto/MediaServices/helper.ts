...
function _validateImgParam(imageType: any, imageCompression: any) {
  ...
  if (!SUPPORTED_IMAGE_TYPES.includes(imageType)) {
    throw new Error(
      imageType + ' is invalid imageType, choose between: ' + SUPPORTED_IMAGE_TYPES.join(', ')
    )
  }
  return true
}

function _getValidImgParam(imageType: any, imageCompression: any) {
  let imgParam: any = {}
  try {
...
export function getImageSize(videoWidth: any, videoHeight: any, sizeFactor: any) {
  // calc the imageWidth
...
export function getDataUri(
  ...
  if (imgParam['imageType'] === IMAGE_TYPES.JPG) {
    ...
    return canvas.toDataURL(FORMAT_TYPES['jpg'], imageCompression)
  }

  return canvas.toDataURL(FORMAT_TYPES[imageType])
}

export function hasConstraints(idealCameraDevice: any, idealResolution: any) {
